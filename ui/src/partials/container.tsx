...
class WmContainerCon extends React.PureComponent<Props> {
  ...
  simLink() {
    if (this.props.sessionId && this.props.active === 'session') {
      ...
      return (<Nav.Item><Nav.Link as="span"><Link to={`/session/${this.props.sessionId}/simulate`}><Octicon name="rocket" /> Simulate</Link></Nav.Link></Nav.Item>)
    } else if (this.props.sessionId && this.props.active === 'simulation') {
      //we are at simulation => create a link to editor pane
      return (<Nav.Item><Nav.Link as="span"><Link to={`/session/${this.props.sessionId}`}><Octicon name="reply" /> Back to Session</Link></Nav.Link></Nav.Item>)
    }
      ...
      <Navbar collapseOnSelect expand="md">
        ...
        <Navbar.Toggle aria-controls="webmodelica-navbar-nav"/>
          ...
          <Navbar.Collapse id="webmodelica-navbar-nav" className="justify-content-end">
            {this.simLink()}
            <Nav.Item as="span"><Link to="/projects"><Octicon name="repo" /> Projects</Link></Nav.Item>
